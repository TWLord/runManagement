...
def runPlotter(ManyArgs, magnet, collatedDict):
    ...
    for key in ManyArgs.keys():
        print key, ':', ManyArgs[key].keys()
        #print 'key:', ManyArgs[key]
        
    datadf = pd.DataFrame(columns=magnetList)
    field_dict = {}
    for magnet in magnetList:
        field_dict[magnet] = []
        print "building list with magnet:", magnet
    for run in ManyArgs.keys():
        #print ManyArgs[run]['xLists'].keys() # adatetime
        #print ManyArgs[run]['yLists'].keys() # afield
        for magnet in magnetList:
            if magnet in ManyArgs[run]['yLists'].keys():
                field_dict[magnet].extend(ManyArgs[run]['yLists'][magnet])
    #datadf.append(field_dict)
    for key in field_dict.keys():
        print "key:", key, "length:", len(field_dict[key])
    #datadf = pd.DataFrame.from_dict(field_dict)
    datadf = pd.DataFrame( {k:pd.Series(v) for k,v in field_dict.iteritems() })
    print datadf 
    #fig, (legendU, SSU, SSD, legendD) = plt.subplots(1, 4, gridspec_kw={"width_ratios":[1,4,4,1]})
    fig = plot_dict["all"]['figure']
    #legendU = plot_dict["all"]['legend_one']
    #legendD = plot_dict["all"]['legend_two']
    SSU = plot_dict["all"]['plot_one']
    SSD = plot_dict["all"]['plot_two']

    #datadf - datadf.mean()
    means = datadf.mean()
    #print means
    datadf_res = datadf - datadf.mean()
    #sns.violinplot(data = datadf, ax=SSU)

    sns.violinplot(data = datadf-datadf.mean(), ax=SSU)
    #sns.pointplot(data = datadf_res, ax=SSU, linestyles='')
    sns.boxplot(data = datadf_res, ax=SSD)
    #sns.scatterplot(data = datadf_res, ax=SSD, alpha=0.5)
    #sns.swarmplot(data = datadf_res, ax=SSD, alpha=0.5)
    #SSD.hist2d(data = datadf)
    #sns.barplot(data = datadf_res, ax=SSD)
    
    outname = ManyArgs[ManyArgs.keys()[-1]]["output"]
    #plot_setup2(fig, SSU, SSD, legendU, legendD, "Hall Probe Residual Over Time", "Hall Probe Residual Over Time", outname) 
    plot_setup_nolegend(fig, SSU, SSD, "Hall Probe Residual Over Time", "Hall Probe Residual Over Time", outname) 
    #plt.show()
...
#
def plot_setup_nolegend(figure, plot_one, plot_two, title_one, title_two, filename):
        plot_one.set_title(title_one)
        plot_two.set_title(title_two)
        plt.tight_layout()
        print "Saving as", filename+"_violin2.png"
        figure.savefig(filename+"_violin2.png", bbox_inches='tight' )
        plt.close(figure)
...
def plot_setup2(figure, plot_one, plot_two, legend_one, legend_two, title_one, title_two, filename):
        ...
        #plt.tight_layout()
        print "Saving as", filename+"_violin2.png"
        figure.savefig(filename+"_violin2.png", bbox_extra_artists=(l1,l2), bbox_inches='tight' )
        plt.close(figure)
...
def plot_setup(figure, aplot, alegend, title, filename):
    ...
    plt.tight_layout()
    plt.show()
    figure.savefig(filename+"violin.png", bbox_extra_artists=(l1,), bbox_inches='tight' )
    plt.close(figure)
...
if __name__ == "__main__":
    ...
    CClist = ["2017-02-6",]
    for CC in CClist: 
        plot_settings.append({"dest":os.path.join("ViolinPlots/",CC),
    ...
    #All = plot_dict['all']
    #plot_setup2(All['figure'], All['plot_one'], All['plot_two'], All['legend_one'], All['legend_two'], "SSU Field Residual Over Runs", "SSD Field Residual Over Runs", 'ViolinPlots/'+out+'_all')
